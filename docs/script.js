...
const LANGS = {
  en: {
    ...
    'hero.lead':'HVNC, DXGI remote desktop H.264 up to 144fps+, remote webcam via DirectShow, process hollowing with PPID spoof, NativeAOT stub — 40+ features. Everything the expensive ones have. None of the price tag.',
    ...
    'dd.gui.p1':'Built on WPF with DevExpress 25.2. A persistent sidebar provides navigation — an icon rail on the left with labelled panels for each feature group. The entire UI adapts to the active theme in real time.',
  ...
  fr: {
    ...
    'hero.lead':'HVNC, bureau distant DXGI H.264 jusqu\'à 144fps+, webcam déportée via DirectShow, process hollowing + PPID spoof, stub NativeAOT — 40+ fonctionnalités. Tout ce qu\'ont les outils payants. Sans le prix.',
    ...
    'dd.hvnc.p3':'Le flux est encodé en temps réel via le H.264 MFT de Windows Media Foundation — sans FFmpeg ni codec externe. Les frames sont capturées depuis le bureau fantôme, converties BGRA→NV12 à la volée et compressées en H.264 Baseline Annex-B avec le mode MF_LOW_LATENCY pour un délai d\'encodage quasi nul. Bascule sur JPEG si MF est indisponible.',
    ...
    'dd.gui.p1':'Construit sur WPF avec DevExpress 25.2. Une barre latérale persistante assure la navigation — un rail d\'icônes à gauche avec des panneaux étiquetés pour chaque groupe de fonctionnalités. Toute l\'interface s\'adapte au thème actif en temps réel.',
  ...
  es: {
    ...
    'hero.lead':'HVNC, escritorio remoto DXGI H.264 hasta 144fps+, webcam vía DirectShow, process hollowing + PPID spoof, stub NativeAOT — más de 40 funciones. Todo lo que tienen los de pago. Sin el precio.',
    ...
    'dd.hvnc.p3':'El stream se codifica en tiempo real usando el H.264 MFT de Windows Media Foundation — sin FFmpeg, sin codec externo. Los fotogramas se capturan desde el escritorio oculto, se convierten BGRA→NV12 al vuelo y se comprimen a H.264 Baseline Annex-B con el modo MF_LOW_LATENCY para un retraso de codificación casi nulo. Vuelve a JPEG si MF no está disponible.',
    ...
    'dd.gui.p1':'Construido sobre WPF con DevExpress 25.2. Una barra lateral persistente gestiona la navegación — un carril de iconos a la izquierda con paneles etiquetados por grupo de funciones. Toda la interfaz se adapta al tema activo en tiempo real.',
  ...
  de: {
    ...
    'hero.lead':'HVNC, DXGI-Ferndesktop H.264 bis 144fps+, Webcam via DirectShow, Process Hollowing + PPID-Spoofing, NativeAOT-Stub — 40+ Funktionen. Alles was die teuren haben. Ohne den Preisschild.',
    ...
    'dd.hvnc.p3':'Der Stream wird in Echtzeit mit dem Windows Media Foundation H.264 MFT kodiert — kein FFmpeg, kein externer Codec. Frames werden vom Shadow-Desktop erfasst, on-the-fly zu BGRA→NV12 konvertiert und im H.264 Baseline Annex-B mit MF_LOW_LATENCY-Modus komprimiert. Fällt auf JPEG zurück, wenn MF nicht verfügbar ist.',
    ...
    'dd.gui.p1':'Aufgebaut auf WPF mit DevExpress 25.2. Eine persistente Seitenleiste übernimmt die Navigation — eine Icon-Leiste links mit beschrifteten Panels für jede Funktionsgruppe. Die gesamte Oberfläche passt sich dem aktiven Theme in Echtzeit an.',
  ...
  pt: {
    ...
    'hero.lead':'HVNC, desktop remoto DXGI H.264 até 144fps+, webcam via DirectShow, process hollowing + PPID spoof, stub NativeAOT — mais de 40 recursos. Tudo que os pagos têm. Sem o preço.',
    ...
    'dd.hvnc.p3':'O stream é codificado em tempo real usando o H.264 MFT do Windows Media Foundation — sem FFmpeg, sem codec externo. Os quadros são capturados do desktop oculto, convertidos BGRA→NV12 em tempo real e comprimidos para H.264 Baseline Annex-B com o modo MF_LOW_LATENCY para atraso de codificação quase zero. Volta para JPEG se MF não estiver disponível.',
    ...
    'dd.gui.h3':'Interface WPF — DevExpress',
    'dd.gui.p1':'Construído sobre WPF com DevExpress 25.2. Uma barra lateral persistente gerencia a navegação — uma trilha de ícones à esquerda com painéis rotulados por grupo de funcionalidades. Toda a interface se adapta ao tema ativo em tempo real.',
  ...
  ru: {
    ...
    'hero.lead':'HVNC, удалённый стол DXGI H.264 до 144fps+, веб-камера через DirectShow, process hollowing + PPID spoof, стаб NativeAOT — 40+ функций. Всё, что есть у платных. Без ценника.',
    ...
    'dd.hvnc.p3':'Поток кодируется в реальном времени через H.264 MFT Windows Media Foundation — без FFmpeg, без внешних кодеков. Кадры захватываются с теневого рабочего стола, конвертируются BGRA→NV12 на лету и сжимаются в H.264 Baseline Annex-B в режиме MF_LOW_LATENCY для минимальной задержки. Переключается на JPEG, если MF недоступен.',
    ...
    'dd.gui.p1':'Построен на WPF с DevExpress 25.2. Постоянная боковая панель обеспечивает навигацию — рейка иконок слева с подписанными панелями для каждой группы функций. Весь интерфейс адаптируется к активной теме в реальном времени.',
  ...
  zh: {
    ...
    'hero.lead':'HVNC、DXGI H.264 远程桌面最高144fps+、DirectShow 网络摄像头、进程镂空 + PPID 欺骗、NativeAOT 存根 — 40+ 功能。付费工具有的全都有，没有价格标签。',
    ...
    'dd.hvnc.p3':'流使用 Windows Media Foundation H.264 MFT 实时编码——无需 FFmpeg，无需外部编解码器。帧从影子桌面捕获，实时转换 BGRA→NV12，以 MF_LOW_LATENCY 模式压缩为 H.264 Baseline Annex-B，编码延迟极低。如果 MF 不可用，则回退到 JPEG。',
    ...
    'dd.gui.p1':'基于 WPF 与 DevExpress 25.2 构建。持久侧边栏提供导航 — 左侧图标栏搭配各功能组的标签面板。整个界面实时响应当前主题。',
  ...
  ja: {
    ...
    'hero.lead':'HVNC、DXGI H.264 リモートデスクトップ最大144fps+、DirectShow ウェブカメラ、プロセスホロウイング + PPIDスプーフ、NativeAOTスタブ — 40以上の機能。高価なツールが持つすべて。価格タグなし。',
    ...
    'dd.hvnc.p3':'ストリームはWindows Media Foundation H.264 MFTを使用してリアルタイムでエンコードされます — FFmpegも外部コーデックも不要。フレームはシャドウデスクトップからキャプチャされ、BGRAからNV12にオンザフライで変換され、MF_LOW_LATENCYモードでH.264 Baseline Annex-Bに圧縮されます。MFが利用できない場合はJPEGにフォールバックします。',
    ...
    'dd.gui.p1':'WPF と DevExpress 25.2 で構築。永続的なサイドバーがナビゲーションを提供 — 左側のアイコンレールと各機能グループのラベル付きパネル。UI 全体がアクティブなテーマにリアルタイムで適応します。',
  ...
  ar: {
    ...
    'hero.lead':'HVNC، سطح مكتب DXGI H.264 حتى 144fps+، كاميرا ويب عبر DirectShow، حقن العمليات + PPID Spoof، stub NativeAOT — أكثر من 40 ميزة. كل ما لدى الأدوات المدفوعة. بدون سعر.',
    ...
    'dd.hvnc.p3':'يُشفَّر الدفق في الوقت الفعلي باستخدام Windows Media Foundation H.264 MFT — بدون FFmpeg وبدون ترميز خارجي. يتم التقاط الإطارات من سطح المكتب الخفي وتحويلها من BGRA إلى NV12 أثناء التشغيل وضغطها إلى H.264 Baseline Annex-B بوضع MF_LOW_LATENCY لتأخير تشفير شبه معدوم. يرجع إلى JPEG إذا لم يتوفر MF.',
    ...
    'dd.gui.p1':'مبني على WPF مع DevExpress 25.2. شريط جانبي دائم يوفر التنقل — شريط أيقونات على اليسار مع لوحات موسومة لكل مجموعة ميزات. تتكيف الواجهة بالكامل مع الثيم النشط في الوقت الفعلي.',
  ...
  it: {
    ...
    'hero.lead':'HVNC, desktop remoto DXGI H.264 fino a 144fps+, webcam remota via DirectShow, process hollowing + PPID spoof, stub NativeAOT — oltre 40 funzionalità. Tutto quello che hanno i tool a pagamento. Senza il cartellino del prezzo.',
    ...
    'dd.hvnc.p3':'Lo stream viene codificato in tempo reale tramite il H.264 MFT di Windows Media Foundation — senza FFmpeg, senza codec esterni. I frame vengono catturati dal desktop ombra, convertiti BGRA→NV12 al volo e compressi in H.264 Baseline Annex-B con la modalità MF_LOW_LATENCY per un ritardo di codifica quasi nullo. Ricade su JPEG se MF non è disponibile.',
    ...
    'dd.gui.p1':'Costruita su WPF con DevExpress 25.2. Una barra laterale persistente gestisce la navigazione — una barra di icone a sinistra con pannelli etichettati per ogni gruppo di funzionalità. L\'intera interfaccia si adatta al tema attivo in tempo reale.',
  ...
  nl: {
    ...
    'hero.lead':'HVNC, DXGI H.264 extern bureaublad tot 144fps+, webcam via DirectShow, process hollowing + PPID spoof, NativeAOT stub — 40+ functies. Alles wat de dure hebben. Zonder het prijskaartje.',
    ...
    'dd.hvnc.p3':'De stream wordt in realtime gecodeerd met de Windows Media Foundation H.264 MFT — geen FFmpeg, geen externe codec. Frames worden vastgelegd van het schaduwbureaublad, on-the-fly omgezet van BGRA→NV12 en gecomprimeerd naar H.264 Baseline Annex-B in de MF_LOW_LATENCY-modus voor een bijna nul encoder-vertraging. Valt terug op JPEG als MF niet beschikbaar is.',
    ...
    'dd.gui.h3':'WPF-interface — DevExpress',
    'dd.gui.p1':'Gebouwd op WPF met DevExpress 25.2. Een vaste zijbalk verzorgt de navigatie — een iconenbalk links met gelabelde panelen per functiegroep. De volledige interface past zich in realtime aan het actieve thema aan.',
  ...
  tr: {
    ...
    'hero.lead':'HVNC, DXGI H.264 uzak masaüstü 144fps\'e kadar, DirectShow ile webcam, process hollowing + PPID spoof, NativeAOT stub — 40+ özellik. Pahalıların sahip olduğu her şey. Fiyat etiketi olmadan.',
    ...
    'dd.hvnc.p3':'Akış, Windows Media Foundation H.264 MFT kullanılarak gerçek zamanlı olarak kodlanır — FFmpeg yok, harici codec yok. Kareler gölge masaüstünden alınır, anında BGRA→NV12\'ye dönüştürülür ve MF_LOW_LATENCY moduyla H.264 Baseline Annex-B\'ye sıkıştırılır. MF kullanılamıyorsa JPEG\'e geri döner.',
    ...
    'dd.gui.p1':'WPF ve DevExpress 25.2 ile oluşturulmuştur. Kalıcı bir kenar çubuğu gezintiyi sağlar — sol tarafta her özellik grubu için etiketli paneller içeren bir simge rayı. Arayüzün tamamı aktif temaya gerçek zamanlı olarak uyum sağlar.',
  ...
  ko: {
    ...
    'hero.lead':'HVNC, DXGI H.264 원격 데스크톱 최대 144fps+, DirectShow 웹캠, 프로세스 할로윙 + PPID 스푸핑, NativeAOT 스텁 — 40개 이상의 기능. 비싼 도구들이 가진 모든 것. 가격표 없이.',
    ...
    'dd.hvnc.p3':'스트림은 Windows Media Foundation H.264 MFT를 사용하여 실시간으로 인코딩됩니다 — FFmpeg 없음, 외부 코덱 없음. 프레임은 그림자 데스크톱에서 캡처되어 즉석에서 BGRA→NV12로 변환되고, MF_LOW_LATENCY 모드로 H.264 Baseline Annex-B로 압축되어 인코더 지연이 거의 없습니다. MF를 사용할 수 없으면 JPEG로 폴백됩니다.',
    ...
    'dd.gui.p1':'WPF와 DevExpress 25.2로 구축되었습니다. 영구 사이드바가 탐색을 제공합니다 — 각 기능 그룹별 레이블 패널이 있는 왼쪽 아이콘 레일. 전체 UI가 활성 테마에 실시간으로 적응합니다.',
  ...
  pl: {
    ...
    'hero.lead':'HVNC, zdalny pulpit DXGI H.264 do 144fps+, kamera przez DirectShow, process hollowing + PPID spoof, stub NativeAOT — ponad 40 funkcji. Wszystko, co mają drogie narzędzia. Bez ceny.',
    ...
    'dd.hvnc.p3':'Strumień jest kodowany w czasie rzeczywistym przy użyciu Windows Media Foundation H.264 MFT — bez FFmpeg, bez zewnętrznych kodeków. Klatki są przechwytywane z ukrytego pulpitu, konwertowane BGRA→NV12 w locie i kompresowane do H.264 Baseline Annex-B w trybie MF_LOW_LATENCY dla niemal zerowego opóźnienia kodera. Powrót do JPEG, gdy MF jest niedostępne.',
    ...
    'dd.gui.p1':'Zbudowany na WPF z DevExpress 25.2. Stały pasek boczny zapewnia nawigację — listwa ikon po lewej stronie z oznaczonymi panelami dla każdej grupy funkcji. Cały interfejs dostosowuje się do aktywnego motywu w czasie rzeczywistym.',
  ...
  id: {
    ...
    'hero.lead':'HVNC, remote desktop DXGI H.264 hingga 144fps+, webcam via DirectShow, process hollowing + PPID spoof, stub NativeAOT — 40+ fitur. Semua yang dimiliki alat berbayar. Tanpa harga.',
    ...
    'dd.hvnc.p3':'Stream dikodekan secara real-time menggunakan Windows Media Foundation H.264 MFT — tanpa FFmpeg, tanpa codec eksternal. Frame diambil dari desktop bayangan, dikonversi BGRA→NV12 secara langsung, dan dikompresi ke H.264 Baseline Annex-B dengan mode MF_LOW_LATENCY untuk penundaan encoder yang hampir nol. Kembali ke JPEG jika MF tidak tersedia.',
    ...
    'dd.gui.p1':'Dibangun di atas WPF dengan DevExpress 25.2. Bilah samping permanen menyediakan navigasi — bilah ikon di sebelah kiri dengan panel berlabel untuk setiap grup fitur. Seluruh antarmuka menyesuaikan diri dengan tema aktif secara real time.',
  ...
  vi: {
    ...
    'hero.lead':'HVNC, màn hình từ xa DXGI H.264 lên đến 144fps+, webcam qua DirectShow, process hollowing + PPID spoof, stub NativeAOT — hơn 40 tính năng. Tất cả những gì công cụ trả phí có. Không có giá.',
    ...
    'dd.hvnc.p3':'Luồng được mã hóa theo thời gian thực bằng Windows Media Foundation H.264 MFT — không có FFmpeg, không có codec ngoài. Các khung hình được chụp từ màn hình nền ẩn, chuyển đổi BGRA→NV12 ngay lập tức và nén thành H.264 Baseline Annex-B với chế độ MF_LOW_LATENCY để gần như không có độ trễ bộ mã hóa. Quay lại JPEG nếu MF không khả dụng.',
    ...
    'dd.gui.p1':'Được xây dựng trên WPF với DevExpress 25.2. Thanh bên cố định cung cấp điều hướng — thanh biểu tượng bên trái với các bảng có nhãn cho từng nhóm tính năng. Toàn bộ giao diện thích nghi với chủ đề đang hoạt động theo thời gian thực.',
  ...
  uk: {
    ...
    'hero.lead':'HVNC, віддалений стіл DXGI H.264 до 144fps+, вебкамера через DirectShow, process hollowing + PPID spoof, стаб NativeAOT — 40+ функцій. Все, що є у платних. Без цінника.',
    ...
    'dd.hvnc.p3':'Потік кодується в реальному часі за допомогою Windows Media Foundation H.264 MFT — без FFmpeg, без зовнішніх кодеків. Кадри захоплюються з тіньового робочого стола, перетворюються з BGRA→NV12 на льоту та стискаються до H.264 Baseline Annex-B у режимі MF_LOW_LATENCY для мінімальної затримки. Повертається до JPEG, якщо MF недоступний.',
    ...
    'dd.gui.p1':'Побудований на WPF з DevExpress 25.2. Постійна бічна панель забезпечує навігацію — рейка іконок ліворуч із підписаними панелями для кожної групи функцій. Весь інтерфейс адаптується до активної теми в реальному часі.',
  ...
  he: {
    ...
    'hero.lead':'HVNC, שולחן עבודה מרוחק DXGI H.264 עד 144fps+, מצלמת רשת דרך DirectShow, process hollowing עם PPID spoof, NativeAOT stub — 40+ תכונות. כל מה שיש לכלים היקרים. בלי תגית המחיר.',
    ...
    'dd.hvnc.p3':'הזרם מקודד בזמן אמת באמצעות Windows Media Foundation H.264 MFT — ללא FFmpeg וללא קודק חיצוני. פריימים נלכדים מהשולחן הנסתר, ממירים BGRA→NV12 תוך כדי ריצה ומדוחסים ל-H.264 Baseline Annex-B במצב MF_LOW_LATENCY לעיכוב קודד כמעט אפס. חוזר ל-JPEG אם MF אינו זמין.',
    ...
    'dd.gui.p1':'בנוי על WPF עם DevExpress 25.2. סרגל צד קבוע מספק ניווט — פס אייקונים בצד שמאל עם לוחות מסומנים לכל קבוצת תכונות. כל הממשק מסתגל לנושא הפעיל בזמן אמת.',
